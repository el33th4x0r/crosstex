...
\item[\texttt{--titlecase=CASE}]
Force titles into the specified case. One can specify \texttt{lower}, for the first letter to be capitalized and everything forced to small case; \texttt{upper}, which forces all letters to upper-case; or \texttt{title}, which capitalizes the first letters of most words, but knows about small words (`a', `the') and certain specific phrases (`BGP', `Peer-to-Peer'). The default, which can be mentioned explicitly with \texttt{--titlecase default}, is to leave titles exactly as in the database.
